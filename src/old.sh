...
if [[ $is_old_use && ! $is_old_header_type ]]; then
    # Não usar TLS automático do Caddy
    [[ ! $caddy ]] && is_old_use=
fi

# Adiciona configuração antiga
if [[ $is_old_use ]]; then
    is_tmp_list=("Excluir configuração antiga" "Restaurar: $is_old")

    ask list is_do_upgrade null "\nDeseja restaurar a configuração antiga:\n"
    ...
    if [[ $caddy ]]; then
        get install-caddy
        # Fazer backup dos arquivos do Caddy
    ...
    ask string y "Deseja excluir a configuração antiga? [y]:"
    _green "\nExclusão concluída!\n"
fi
